...
import com.viettel.crm.cloudgateway.modal.UserDto;
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            return webClientBuilder.build()
                    .post()
                    .uri(authTokenUrl+"?token=" + authHeader)
                    .retrieve()
                    .bodyToMono(UserDto.class)
                    .map(userDto -> {
                        exchange.getRequest()
                                .mutate()
                                .header("uid", String.valueOf(userDto.getId()))
                                .header("uname", String.valueOf(userDto.getUserName()));
                        return exchange;
